...
public class Recipes {
	...
	public static JsonObject createShapedRecipeJson(ArrayList<Character> keys, ArrayList<Identifier> items,
			ArrayList<String> type, ArrayList<String> pattern, Identifier output, int count) {
		...
		// Next we need to define what the keys in the pattern are. For this we need
		// different JsonObjects per key definition, and one main JsonObject that will
		// contain all of the defined keys.
		...
		for (int i = 0; i < keys.size(); ++i) {
			...
			individualKey.addProperty(type.get(i), items.get(i).toString()); // This will create a key in the form
																				// "type": "input", where type is either
																				// "item" or "tag", and input is our
																				// input item.
